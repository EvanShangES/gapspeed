angular.module('mrbaffo.user.order').controller('editOrderController', function($scope, $rootScope, $http, APP_CONFIG, $stateParams, $q){
    ...
    $scope.loadOrder = function(id){
        ...
        var req = {
            method: "GET",
            url: APP_CONFIG.apiUrl + "/order/loadOrder/" +id
        };

        $http(req).then(function (res) {
            console.log(res.data);
            if(res.data.status){
                currentOrder.data = res.data.order;
                dfd.resolve(currentOrder);
            }
        });
    ...
    var loadedCurrentOrder = $scope.loadOrder($stateParams.orderId);
    loadedCurrentOrder.initialized.then(function(){
        $scope.currentOrder = loadedCurrentOrder.data;
        // console.log($scope.currentOrder);
        for(var i =0; i <  $scope.currentOrder.orderInfo.products.length; i++){
            var name = $scope.currentOrder.orderInfo.products[i].name;
            console.log(name);
            if(name == 'Wash'){
                $scope.serviceSelection.wash = !$scope.serviceSelection.wash;
            }
            if(name == 'Shirt (Laundry)'){
                $scope.serviceSelection.shirt.selected = true;
                $scope.serviceSelection.shirt.press = "Laundry & Press";
            }
            if(name == 'Shirt (Hand Press)'){
                $scope.serviceSelection.shirt.selected = true;
                $scope.serviceSelection.shirt.press = 'Hand Press';
            }
            if(name == 'Shirt (Dry Clean)'){
                $scope.serviceSelection.shirt.selected = true;
    ...
    $scope.confirmAddress = function(address, type){
        ...
        var req = {
            method: 'GET',
            url: APP_CONFIG.apiUrl + '/addressCheck/' + address.address + '/' + address.city
        };
        ...
        if(JSON.stringify($rootScope.userInfo.cleanCloudInfo.lastAddress) !== JSON.stringify($scope.request.address)) {
            ...
            $http(req).then(function(res){
                ...
                if(res.data.Success === 'True'){
                    editOrder($scope.currentOrder.orderId);
                //     // editPromo($scope.usedPromo);
                }
            })
        }
    };
    ...
    $scope.sendOrder = function (){
        console.log($scope.request);
        if(!$rootScope.userInfo.cleanCloudInfo.id){
            //adding new customer
            var req = {
                method: "POST",
    ...
    function editOrder(orderId){
        ...
        var req = {
            method: "POST",
            url: APP_CONFIG.apiUrl + "/order/updateOrder/" + orderId,
            data: {
                'orderInfo':   {
                    'products': $scope.request.products,
